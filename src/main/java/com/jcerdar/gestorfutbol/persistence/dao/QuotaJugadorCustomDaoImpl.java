...
@Repository
public class QuotaJugadorCustomDaoImpl implements QuotaJugadorCustomDao{

    @PersistenceContext
    private EntityManager entityManager;

    @Override
    public Page<QuotaJugador> buscarConFiltros(Filtre filtre) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();

        // MAIN QUERY
        CriteriaQuery<QuotaJugador> query = cb.createQuery(QuotaJugador.class);
        Root<QuotaJugador> root = query.from(QuotaJugador.class);
        ...
        TypedQuery<QuotaJugador> typedQuery = entityManager.createQuery(query);
        ...
        List<QuotaJugador> resultList = typedQuery.getResultList();

        // COUNT QUERY
        CriteriaQuery<Long> countQuery = cb.createQuery(Long.class);
        Root<QuotaJugador> countRoot = countQuery.from(QuotaJugador.class);
    ...
    @Override
    public List<QuotaJugador> buscarConFiltrosAll(Filtre filtre) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
    ...
    private List<Predicate> construirPredicats(Filtre filtre, Root<QuotaJugador> root, CriteriaBuilder cb) {
        List<Predicate> predicates = new ArrayList<>();
        Join<QuotaJugador, Jugador> jug = root.join("jugador", JoinType.INNER);

        if (filtre.getCampanyaActiva() != null) {
            predicates.add(cb.equal(jug.get("campanya").get("id"), filtre.getCampanyaActiva()));
        }

        if (filtre.getEquipActiu() != null) {
            predicates.add(cb.equal(jug.get("equip").get("id"), filtre.getEquipActiu()));
        }
                ...
                if (value != null && !value.isEmpty()) {
                    if ("nomComplet".equals(field)) {
                        ...
                        Expression<String> nomComplet = cb.concat(
                                cb.concat(cb.lower(jug.get("nom")), " "),
                                cb.concat(cb.lower(jug.get("llinatge1")), " ")
                        );
                        nomComplet = cb.concat(nomComplet, cb.lower(jug.get("llinatge2")));
                        ...
                        switch (matchMode) {
                            ...
                            case "equals" -> {
                                if (field.equals("tipoQuotaJugador")) {
