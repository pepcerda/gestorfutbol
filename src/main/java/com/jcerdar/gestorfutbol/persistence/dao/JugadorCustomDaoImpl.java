...
@Repository
public class JugadorCustomDaoImpl implements JugadorCustomDao {

    @PersistenceContext
    private EntityManager entityManager;

    @Override
    public Page<Jugador> buscarConFiltros(Filtre filtre) {
        ...
        TypedQuery<Jugador> typedQuery = entityManager.createQuery(query);
        ...
        List<Jugador> resultList = typedQuery.getResultList();

        // COUNT QUERY
        CriteriaQuery<Long> countQuery = cb.createQuery(Long.class);
        Root<Jugador> countRoot = countQuery.from(Jugador.class);
    ...
    @Override
    public List<Jugador> buscarConFiltrosAll(Filtre filtre) {
        CriteriaBuilder cb = entityManager.getCriteriaBuilder();
    ...
    private List<Predicate> construirPredicats(Filtre filtre, Root<Jugador> root, CriteriaBuilder cb) {
        ...
        if (filtre.getEquipActiu() != null) {
            predicates.add(cb.equal(root.get("equip").get("id"), filtre.getEquipActiu()));
        }
